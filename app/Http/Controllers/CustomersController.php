...
class CustomersController extends Controller
{
    ...
    public function customerList() {
        ...
        foreach ($res as $row) {
            ...
            $tempRow['profile'] = ($row->profile != '') ? '<a class="image-popup-no-margins" href="' .url('images') . config('global.USER_IMG_PATH')  . $row->profile . '" width="55" height="55"><img class="rounded avatar-md shadow img-fluid" alt="" src="' . url('images'). config('global.USER_IMG_PATH')  . $row->profile . '" width="55" height="55"></a>' : '';
